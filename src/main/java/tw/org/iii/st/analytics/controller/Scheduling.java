...
public class Scheduling {
	...
	private @ResponseBody List<TourEvent> StartPlan(@RequestBody SchedulingInput json) throws ParseException, ClassNotFoundException, SQLException
	{		
		...
		if ("".equals(si.getEndPoiId()) || si.getEndPoiId() == null)
		{
			...
			{
				System.out.println(t.getPoiId() + "," + t.getStartTime() + "," + t.getEndTime());
			}
		...
		else
		{
			...
			freetime = FreeTime(PlanResult.get(2).getEndTime(),sdf.parse(si.getEndTime()));
			getOtherPOI(freetime);
			for (TourEvent t : PlanResult)
			{
				System.out.println(t.getPoiId() + "," + t.getStartTime() + "," + t.getEndTime());
	...
	private ArrayList<TourEvent> findTop(int freetime) throws ParseException, SQLException
	{
		...
		Calendar cal;
		Date date = sdf.parse(si.getStartTime());
		...
		for (schedulingInfo i : rs)
		{
			...
			{
				dis = Distance(i.getPy(),i.getPx(),Double.parseDouble(gps[1]),Double.parseDouble(gps[0]));
			}
			else
			{
				List<GPS> gp = QGPS("SELECT px,py FROM scheduling WHERE place_id = '"+si.getStartPoiId()+"'");
				if (gp.get(0).getX()==0 || gp.get(0).getY()==0)
					dis = Distance(i.getPy(),i.getPx(),Double.parseDouble(gps[1]),Double.parseDouble(gps[0]));
				else
					dis = Distance(i.getPy(),i.getPx(),gp.get(0).getY(),gp.get(0).getX());
			}
			
			Date startTime = sdf.parse(si.getStartTime());
			if (!(startTime.getHours() >= 11 && startTime.getHours() <= 13) && !(startTime.getHours() >= 17 && startTime.getHours() <= 19))
			{
				if (i.getPreference().equals("PF1") || "".equals(i.getPreference()) || i.getPreference() == null)
					continue;
			}
	...
	{
		List<schedulingInfo> results = jdbcTemplate.query(
				q,
				new RowMapper<schedulingInfo>() {
					@Override
					public schedulingInfo mapRow(ResultSet rs, int rowNum)
							throws SQLException {
						return new schedulingInfo(rs.getString("A.place_id"),rs.getString("A.preference"),rs.getInt("stay_time"),rs.getDouble("px"),rs.getDouble("py"));
					}
				});
		return results;
	...
	{
		List<GPS> results = jdbcTemplate.query(
				q,
				new RowMapper<GPS>() {
					@Override
					public GPS mapRow(ResultSet rs, int rowNum)
							throws SQLException {
...
//		rs = sc.select_table("SELECT A.place_id,stay_time,px,py FROM scheduling AS A,OpenTimeArray AS B WHERE A.region = '"+r+"' and A.mission = 'A' and A.stay_time < "+(freetime*20)+" and "
//				+ "A.Place_Id = B.place_id and B.weekday = '"+weekday +"' and B."+date.getHours()+"_Oclock = 1 ORDER BY rand()");
//		rs.next();
//		
//		result
		TourEvent te;
	...
	private TourEvent costTime(Date startTime,String now,String destination,int range,int checkin) throws SQLException, ParseException
	{
...
//						+ "AND B."+startTime.getHours()+"_Oclock = 1 ORDER BY RAND() DESC");
		for (SchedulingDis sd : rs) //找出目前所在景點可考慮去的鄰近景點候選清單
		{
			...
			{
				if (!repeat.contains(sd.getPlaceID()) && (sd.getTotalTime())<lastTime) //達成任務就離開
				{
					te.setPoiId(sd.getPlaceID());
					te.setStartTime(addTime(PlanResult.get(index-1).getEndTime(),sd.getTime()));
					te.setEndTime(addTime(PlanResult.get(index-1).getEndTime(),sd.getTotalTime()));
			...
			else
			{
				...
				if (!repeat.contains(sd.getPlaceID()) && (destinationTime+sd.getTotalTime())<lastTime) //達成任務就離開
				{
					te.setPoiId(sd.getPlaceID());
					te.setStartTime(addTime(PlanResult.get(index-1).getEndTime(),sd.getTime()));
					te.setEndTime(addTime(PlanResult.get(index-1).getEndTime(), sd.getTotalTime()));
	...
	{
		List<SchedulingDis> results = jdbcTemplate.query(
				q,
				new RowMapper<SchedulingDis>() {
					@Override
					public SchedulingDis mapRow(ResultSet rs, int rowNum)
							throws SQLException {
						return new SchedulingDis(rs.getString("A.arrival_id"),rs.getString("A.preference"),rs.getInt("checkins"),rs.getInt("time"),rs.getInt("totalTime"));
					}
				});
		return results;
	...
	private ArrayList<TourEvent> getMission(int freetime) throws SQLException, ClassNotFoundException, ParseException
	{
		...
		String gps[] = si.getGps().split(",");
		double dis;
//		dis = Distance(rs.get(0).getPy(),rs.get(0).getPx(),Double.parseDouble(gps[1]),Double.parseDouble(gps[0]));
		if ("".equals(si.getStartPoiId()) || si.getStartPoiId() == null)
		{
			dis = Distance(rs.get(0).getPy(),rs.get(0).getPx(),Double.parseDouble(gps[1]),Double.parseDouble(gps[0]));
		}
		else
		{
			...
			if (gp.get(0).getX()==0 || gp.get(0).getY()==0)
				...
				dis = Distance(rs.get(0).getPy(),rs.get(0).getPx(),gp.get(0).getY(),gp.get(0).getX());
		}
		...
		te = new TourEvent();
		te.setPoiId(rs.get(0).getPlaceID());
		te.setStartTime(addTime(result.get(index-1).getEndTime(),BetweenTime(result.get(index-1).getPoiId(),rs.get(0).getPlaceID())));
		te.setEndTime(addTime(te.getStartTime(),30));
